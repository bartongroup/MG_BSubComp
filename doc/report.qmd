...
## Annotating non-coding RNA {#sec-annotating-ncrna}
...
## Mapping gene names between *SubtiWiki* and NCBI {#sec-map-subtiwiki}

The gene symbols (names) used by NCBI diverge from the *SubtiWiki* names to which Stanley-Wall lab are accustomed to. Therefore an attempt was made to map *SubtiWiki* genes to NCBI and replace the differing gene symbols where possible. A similar tactic to that used in @sec-annotating-ncrna was applied. Sequences for all genes were extracted from *SubtiWiki* and then mapped to the NCBI genome sequence (chromosome only). The non-unique mappings were removed -- these corresponded to ribosomal sequences. Uniquely mapped sequences were then matched by coordinates (start, end, strand) to the NCBI annotations. Summary of matching statistics are shown in @tbl-subtiwiki-matching-stats.

```{r}
#| label: tbl-subtiwiki-matching-stats
#| tbl-cap: Results for mapping *SubtiWiki* genes to NCBI genome.
tar_read(sw_match_stats) |> 
  gt() |> 
  fmt_number(
    columns = "Percentage",
    n_sigfig = 3
...
Where mapping was found, the *SubtiWiki* gene symbols are used, everywhere else the original NCBI gene symbols were left.

::: {style="height: 600px; width: fit-content; overflow-y: scroll; margin-left: auto; margin-right: auto; font-size: 10pt"}

```{r}
#| label: tbl-subtiwiki-non-matching
#| tbl-cap: NCBI genes for which either *SubtiWiki* gene name was different or where no *SubtiWiki* counterparts were found.
tar_read(genes) |>
  dplyr::filter(chr == "NZ_CP020102.1") |> 
  dplyr::filter(ncbi_gene_symbol != sw_gene_symbol | is.na(sw_gene_symbol)) |>
  dplyr::mutate(across(everything(), ~replace_na(.x, "--"))) |> 
  dplyr::arrange(ncbi_gene_symbol) |> 
  dplyr::select(`NCBI ID` = id, `NCBI symbol` = ncbi_gene_symbol, `SubtiWiki symbol` = sw_gene_symbol, Biotype = biotype, Description = description) |> 
  gt()
```
...
```{r}
#| label: tbl-fscreen
#| tbl-cap: FastQ screen one hit per genome counts. FastQ screen samples 100,000 random reads from each FASTQ file to do the mapping.
tar_read(fscrn) |>
  filter( name == "#One_hit_one_genome") |>
  pivot_wider(id_cols = Genome, names_from = sample, values_from = value) |>
  arrange(desc(ctrl_1)) |> 
  gt_table()
```
...
A list of `r n_toxan + 1` toxins and antibiotics was provided. *pksK* was removed from the list, as it was found to be a synonym of *pksJ*, already on the list. The remaining genes are listed in @tbl-toxans.

```{r}
#| label: tbl-toxans
#| tbl-cap: List of toxins and antibiotics. Gene symbols provided were matched to the *SubtiWiki* gene symbols, mapped to NCBI annotations (see @sec-map-subtiwiki).
toxan |> 
  select(-ncbi_gene_symbol) |> 
  gt()
```
...
- [DE genome browser](https://shiny.compbio.dundee.ac.uk/mgierlinski/private/bsub_comp/browser)
