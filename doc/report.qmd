...
Sample `mut_5` shows strong ribosomal contamination. @fig-bad-sample shows a comparison of this sample with `mut_1`, which does not show strong contamination. @fig-good-sample shows two good samples for context. We see that apart from slightly increased noise, the main difference is in the expression of rRNA genes.
...
Only genes with at least 10 reads in at least one sample are used for differential expression. *edgeR* was used for differential expression between mutant and control.
